...
def search_bgg_by_author(author_name, max_retries=3):
    """
    ...
    """
    # Step 1: Find the designer ID using documented API
    ...
    for attempt in range(max_retries):
        try:
            ...
            try:
                ...
                # Use the first matching designer
            ...
            # Step 2: Get games by designer using undocumented API
            ...
            for games_attempt in range(max_retries):
                try:
                    ...
                    tqdm.write(f"Games request failed for designer {designer_id} (attempt {games_attempt + 1}): {e}")
                    if games_attempt < max_retries - 1:
                        time.sleep(2)
            
            return []
            
        except requests.exceptions.RequestException as e:
            tqdm.write(f"Designer search failed for '{author_name}' (attempt {attempt + 1}): {e}")
            if attempt < max_retries - 1:
                time.sleep(1)
